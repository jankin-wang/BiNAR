...
// 将每个高斯的输入球面谐波系数转换为简单 RGB 颜色的前向方法。
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	...
	// RGB 颜色被限制为正值。如果值被限制，我们需要跟踪它以进行反向传递。
...
__device__ float4 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, float kernel_size, const float* cov3D, const float* viewmatrix)
{
	...
	//应用低通滤波器：每个高斯函数应至少为一个像素宽/高。丢弃第三行和第三列。
...
// 将每个高斯的尺度和旋转属性转换为世界空间中的 3D 协方差矩阵的前向方法。还考虑了四元数归一化。
	...
	bool prefiltered)
{
	...
	// 将半径和接触的图块初始化为 0。如果不改变，此高斯将不会被进一步处理。
	radii[idx] = 0;
	tiles_touched[idx] = 0;
	...
	// 如果 3D 协方差矩阵是预先计算的，则使用它，否则根据缩放和旋转参数计算。
	const float* cov3D;
	...
	// 计算屏幕空间中的范围（通过查找 2D 协方差矩阵的特征值）。使用范围计算此高斯重叠的屏幕空间图块的边界矩形。如果矩形覆盖 0 个图块，则退出。
	...
	// 如果颜色已预先计算，则使用它们，否则将球面谐波系数转换为 RGB 颜色。
...
// 主要光栅化方法。协作处理每个块的一个图块，每个线程处理一个像素。在获取和光栅化数据之间交替进行。
	...
	float* __restrict__ out_color)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// 使用圆锥矩阵重新采样（参见 Zwicker 等人的“表面光栅化”，2001 年）
			...
			// 等式 (2) 来自 3D 高斯溅射论文。通过与高斯不透明度相乘获得 alpha及其与平均值的指数衰减。避免数值不稳定性（参见论文附录）。
	...
	// 所有处理有效像素的线程都会将其最终的渲染数据写到帧和辅助缓冲区。
	if (inside)
	{
		final_T[pix_id] = T;
